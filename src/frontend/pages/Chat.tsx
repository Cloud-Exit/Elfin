...
function sourceUrl(filename: string): string {
  ...
  return `/api/sources/${encodeURIComponent(filename)}?token=${encodeURIComponent(token)}`
}
...
function renderWithCitations(text: string): ReactNode {
  ...
  while ((match = CITATION_REGEX.exec(text)) !== null) {
    ...
    nodes.push(
      <a
        key={`${match.index}-${filename}`}
        href={sourceUrl(filename)}
        target="_blank"
        rel="noreferrer"
        style={{
          color: '#66ffcc',
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <div className="card" style={{ flex: 1, overflowY: 'auto', padding: '1rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {!selectedSessionId ? (
              ...
              <div className="placeholder" style={{ margin: 'auto', textAlign: 'center' }}>
                No messages yet. Start a conversation with Elfin!
              </div>
            ) : (
              messages.map(m => (
                <div 
                      ...
                      {(() => {
                        try {
                          ...
                          return parsedSources.map((src, idx) => {
                            ...
                            const filename = fullSource.split('#')[0]
                            ...
                            return (
                              <div key={idx} style={{
                                ...
                                borderRadius: '4px',
                                marginBottom: '0.5rem',
                                borderLeft: '2px solid rgb(var(--alt))'
                              }}>
                                <div style={{ fontWeight: 'bold', color: 'rgb(var(--alt))', marginBottom: '0.25rem' }}>
                                  {isOpenable ? (
                                    <a
                                      href={sourceUrl(filename)}
                                      target="_blank"
                                      rel="noreferrer"
                                      style={{ color: '#66ffcc', textDecoration: 'underline', textDecorationThickness: '2px', cursor: 'pointer', textShadow: '0 0 4px rgba(102, 255, 204, 0.6)' }}
                                      title={`Open ${filename}`}
                                    >
                                      [{fullSource}]
                                    </a>
